...
@section('content')
    <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <x-common.component-card title="Edit Pembayaran">
            ...
            <form action="{{ route('admin.pembayaran.update', $pembayaran) }}" method="post" class="space-y-5">
                @csrf
                @method('PUT')
                    ...
                    <div x-data="{ isOptionSelected: true }" class="relative z-20 bg-transparent">
                        <select name="peserta_id" required
                            ...
                            @foreach($pesertas as $ps)
                                <option value="{{ $ps->id }}" @if($ps->id == $pembayaran->peserta_id) selected @endif>
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
                    ...
                    <div>
                        ...
                        <div x-data="{ isOptionSelected: true }" class="relative z-20 bg-transparent">
                            <select name="metode" required
                                class="shadow-theme-xs h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 pr-11 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                                <option value="tunai" @if($pembayaran->metode == 'tunai') selected @endif>
                                    Tunai
                                </option>
                                <option value="transfer" @if($pembayaran->metode == 'transfer') selected @endif>
                                    Transfer
                                </option>
                            </select>
                    ...
                    <div>
                        ...
                        <input name="tanggal" type="date" value="{{ $pembayaran->tanggal }}" required
